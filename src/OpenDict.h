...
constexpr int HASH_MASK = 0xFFFFFFFF; //only lower 32 bits.
...
public:
	...
	uint16_t distance = TOO_FAR_TO_REACH; //<from expected position. all 1's means empty. max distance 64K-2
	uint16_t key_size = 0; //the length of the key. <=8 will be embedded directly. otherwise a pointer 8K max enough?
	uint32_t hash = 0; //lower 4-byte of the 8-byte long hash. the part to calculate position in the table.

	void* value = nullptr; //for Bro, value is always a pointer.
	union{
	...
	void SetEmpty()	 { distance = TOO_FAR_TO_REACH; }
